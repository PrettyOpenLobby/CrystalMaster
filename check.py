...
DENY_HASHES = {
    ...
    "85dd9f88c9230884ba2c5238ada4580c3e6d393e0b19134971b66dcc181ac6c6",
}
TOKEN_RE = re.compile(r"[a-z0-9.@-]{3,64}")

# The board's fonts are OFL-licensed (their licence texts sit beside them) and
# derived under the OFL's Reserved Font Name rules; they are the one font
# family this tree may carry.
FONT_ALLOW_DIR = os.path.join("services", "boardart", "tm")
FONT_ALLOW_PREFIX = "tm-"


def _font_allowed(rel):
    d, fn = os.path.split(rel)
    return (os.path.normpath(d) == os.path.normpath(FONT_ALLOW_DIR)
            and fn.startswith(FONT_ALLOW_PREFIX)
            and os.path.splitext(fn)[1].lower() in (".ttf", ".woff2"))
...
def scan_tree(root):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fn in filenames:
            ...
            if ext in BANNED_EXT and not _font_allowed(rel):
