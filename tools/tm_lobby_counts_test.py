...
Asserted against `tmtitle._lobby_counts_live` itself, reading the counts back
...
# 3 when this test was written and 16 the same evening (when the SE-accurate
# 16-table room was authored), and a literal here turns every authoring change into a
...
out = R._lobby_counts_live("b/g/RL000", RL)
# "left alone" means EQUAL TO THE INPUT, not equal to a literal: the fixture's
# authored value has changed once already and the literal 1 went stale with it.
check(struct.unpack_from("<I", out, _tm.RL_HDR + _tm.RL_F_TABLES)[0]
      == struct.unpack_from("<I", RL, _tm.RL_HDR + _tm.RL_F_TABLES)[0],
